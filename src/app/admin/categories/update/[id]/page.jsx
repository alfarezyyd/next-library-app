...
export default function Page() {
  ...
  const fetchExistingCategory = async (id) => {
    try {
      ...
      if (response.ok) {
        ...
        setPayloadFormData(responseBody['result']['data']);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    setUserError({});
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}categories/${existingCategory.id}`, {
        method: 'PUT',
        includeCredentials: true,
        headers: {
          Accept: 'application/json',
  ...
  return (<Wrapper additionalClass={"font-fraunces bg-white"}>
      <div className="pt-8 flex flex-col gap-2 items-center pb-4 bg-[#3149BB]">
        ...
        <h1 className="font-fraunces text-3xl text-center font-bold text-white">Update Category</h1>
      </div>
